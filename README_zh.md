...
这个仓库记录的是我实际怎么用 AI——真实的工具、真实的流程、真实的习惯。当我的实际使用方式发生变化（换了 MCP 服务器、换了模型、某个工作流不再适用），跑一遍 `/update-workflow`，仓库就会追上现状。实际行为才是真相，文档只是镜像。当活的工作日志读就好——文档追随实际行为，而不是反过来规定行为。
